...
<head>
    ...
    <title>資產管理專家 v3.7.1 - 視覺校正版</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <style>
        body { background-color: #f8fafc; font-family: 'PingFang TC', 'Microsoft JhengHei', sans-serif; }
        .card { background: white; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1); }
        /* 支出/收入 邊框定義 */
        .border-outflow { border-left: 5px solid #ef4444; } /* 買入、費用 */
        .border-inflow { border-left: 5px solid #10b981; }  /* 賣出、收入 */
        input[type="date"], input[type="number"], input[type="text"] { 
            border: 1px solid #e2e8f0; border-radius: 6px; padding: 4px 8px;
        }
        .stock-tag { background: #f1f5f9; color: #475569; padding: 2px 12px; border-radius: 99px; font-weight: bold; font-family: monospace; }
    </style>
</head>
<body class="p-4 md:p-8">
    ...
    <div class="max-w-7xl mx-auto">
        <header class="mb-8 flex justify-between items-end border-b pb-4">
            <div>
                <h1 class="text-3xl font-extrabold text-slate-800">📊 投資流水管理 <span class="text-sm font-normal text-slate-400">v3.7.1 Pro</span></h1>
                <p class="text-slate-500 text-sm mt-1">支出統一標紅：買入/費用 | 收入統一標綠：賣出/收入</p>
            </div>
            <button onclick="exportCSV()" class="bg-slate-700 text-white px-4 py-2 rounded-lg hover:bg-black transition text-sm shadow-sm">導出 CSV</button>
        </header>

        <div id="statsGrid" class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8"></div>

        <div class="card p-6 mb-8 border-t-8 border-slate-800 shadow-md">
            <form id="mainForm" class="grid grid-cols-1 md:grid-cols-7 gap-4 items-end">
                <div class="md:col-span-1">
                    ...
                    <input type="date" id="entryDate" class="w-full bg-white outline-none focus:ring-2 focus:ring-blue-400">
                </div>
                <div class="md:col-span-1">
                    <label class="block text-xs font-bold text-slate-500 mb-2">類型</label>
                    <select id="entryType" onchange="toggleFields()" class="w-full border p-2 rounded-md bg-slate-50 font-medium outline-none">
                ...
                <div class="md:col-span-1">
                    <label class="block text-xs font-bold text-slate-500 mb-2">代號</label>
                ...
                <div class="md:col-span-1">
                    ...
                    <input type="number" id="feeOrAmount" step="0.01" value="0" class="w-full bg-blue-50 outline-none font-bold text-blue-700">
                </div>
                <div class="md:col-span-1">
                    <button type="submit" class="w-full bg-blue-600 text-white py-2 rounded-md hover:bg-blue-700 font-bold transition">記錄</button>
                </div>
                <div class="md:col-span-7">
                    <input type="text" id="note" placeholder="備註..." class="w-full italic text-sm outline-none focus:ring-2 focus:ring-blue-400">
                </div>
            </form>
        </div>
        ...
        <div class="flex justify-between items-center mb-4 px-2">
            ...
            <input type="text" id="filterInput" onkeyup="render()" placeholder="🔍 輸入代號查看持倉與成本..." class="p-2 border rounded-lg text-sm w-72 focus:ring-2 focus:ring-amber-400 outline-none">
        </div>
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-slate-800 text-white">
                    <tr class="uppercase text-[11px] tracking-widest">
                        ...
                        <th class="px-6 py-4 text-left font-medium">代號</th>
                        <th class="px-6 py-4 text-right font-medium">詳情</th>
                        <th class="px-6 py-4 text-right font-medium">附加金額</th>
                        <th class="px-6 py-4 text-right font-medium">現金流</th>
                        <th class="px-6 py-4 text-center font-medium">操作</th>
                    </tr>
                </thead>
                <tbody id="recordBody" class="divide-y divide-slate-100"></tbody>
            </table>
        </div>
    </div>
    ...
    <script>
            ...
            if (filter) {
                ...
                statsGrid.innerHTML = `
                    <div class="card p-5 border-b-4 border-blue-500">
                        <p class="text-[10px] font-bold text-slate-400 uppercase">目前持倉股數</p>
                    ...
                    <div class="card p-5 border-b-4 border-amber-500">
                        <p class="text-[10px] font-bold text-slate-400 uppercase">平均買入成本 (含費)</p>
                    ...
                    <div class="card p-5 border-b-4 border-slate-300">
                        <p class="text-[10px] font-bold text-slate-400 uppercase">累計附加 (息/費)</p>
                ...
                statsGrid.innerHTML = `
                    <div class="card p-5 border-b-4 border-slate-300">
                        <p class="text-[10px] font-bold text-slate-400 uppercase">全帳戶流水淨值</p>
                        <p class="text-2xl font-bold text-slate-800">$${totalCashFlow.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
                    </div>
                    <div class="card p-5 border-b-4 border-red-400">
                        <p class="text-[10px] font-bold text-slate-400 uppercase">總計支出費用</p>
                        <p class="text-2xl font-bold text-red-500">$${totalFees.toLocaleString()}</p>
                    </div>
                    <div class="md:col-span-2 card p-5 bg-slate-800 text-white text-center flex flex-col justify-center">
                        <p class="text-xs font-bold text-slate-300 italic">請輸入「股票代號」以查看持倉統計</p>
                    </div>
                `;
            }
        }
        ...
        function renderNormalRow(r, cash) {
            // 統一核心邏輯：支出類(買入、費用)為紅，收入類(賣出、收入)為綠
            const isOutflow = (r.type === '買入' || r.type === '費用');
            const colorClass = isOutflow ? 'text-red-500' : 'text-green-600';
            const borderClass = isOutflow ? 'border-outflow' : 'border-inflow';

            return `
                <tr class="hover:bg-slate-50 transition ${borderClass}">
                    <td class="px-6 py-4 font-mono text-xs text-slate-500">${r.date}</td>
                    <td class="px-6 py-4 font-bold ${colorClass}">${r.type}</td>
                    <td class="px-6 py-4 font-mono font-bold text-slate-700">${r.symbol}</td>
                    <td class="px-6 py-4 text-right text-slate-500">${r.price ? `${r.price.toLocaleString()} x ${r.quantity}` : '--'}</td>
                    ...
                    <td class="px-6 py-4 text-right font-bold ${cash >= 0 ? 'text-green-600' : 'text-red-500'}">
                        ${cash >= 0 ? '+' : ''}${cash.toLocaleString(undefined, {minimumFractionDigits: 2})}
                    </td>
                    <td class="px-6 py-4 text-center">
                        <button onclick="startEdit(${r.id})" class="text-slate-400 hover:text-blue-600 mr-4 transition">✎</button>
                        <button onclick="deleteRecord(${r.id})" class="text-slate-300 hover:text-red-500 transition">✕</button>
                    </td>
                </tr>
                ${r.note ? `<tr><td colspan="7" class="px-12 py-1 text-[11px] text-slate-400 italic bg-slate-50/30 border-none">└ ${r.note}</td></tr>` : ''}
            `;
        }
        ...
        function renderEditRow(r) {
            return `
                <tr class="bg-amber-50">
                    ...
                    <td class="px-4 py-2">
                        <select id="editType" class="w-full border rounded text-xs">
                            <option value="買入" ${r.type==='買入'?'selected':''}>買入</option>
                            <option value="賣出" ${r.type==='賣出'?'selected':''}>賣出</option>
                            <option value="費用" ${r.type==='費用'?'selected':''}>費用</option>
                            <option value="收入" ${r.type==='收入'?'selected':''}>收入</option>
                        </select>
                    </td>
                    <td class="px-4 py-2"><input type="text" id="editSymbol" value="${r.symbol}" class="w-full uppercase text-xs"></td>
                    ...
                    <td colspan="2" class="px-4 py-2 text-center">
                        <button onclick="saveEdit(${r.id})" class="text-blue-600 font-bold mr-2 text-xs">儲存</button>
                        <button onclick="cancelEdit()" class="text-slate-400 text-xs">取消</button>
                    </td>
                </tr>
            `;
        }
