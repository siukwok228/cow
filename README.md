...
<head>
    ...
    <title>資產管理專家 v3.7.3 - 視覺終極修復版</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <style>
        /* 強制全局背景與字體顏色，對抗瀏覽器插件與深色模式 */
        body { background-color: #f8fafc !important; color: #0f172a !important; font-family: 'PingFang TC', 'Microsoft JhengHei', sans-serif; }
        .card { background-color: #ffffff !important; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1); }
        
        /* 表頭強制顏色：防止白底白字 */
        .force-header { background-color: #1e293b !important; color: #ffffff !important; }
        .force-header th { color: #ffffff !important; font-weight: 800 !important; }
...
<body class="p-4 md:p-8">
    ...
    <div class="max-w-7xl mx-auto">
        <header class="mb-8 flex justify-between items-end border-b-2 border-slate-200 pb-4">
            <div>
                <h1 class="text-3xl font-extrabold text-slate-900">📊 投資流水管理 <span class="text-sm font-normal text-slate-400">v3.7.3 Fix</span></h1>
                <p class="text-slate-600 text-sm mt-1 font-bold">紅色 = 支出 (買入/費用) | 綠色 = 收入 (賣出/股息)</p>
            </div>
            <button onclick="exportCSV()" class="bg-slate-900 text-white px-6 py-2 rounded-lg hover:bg-black transition text-sm font-black shadow-lg">導出 CSV</button>
        </header>
        ...
        <div class="card p-6 mb-8 border-t-8 border-slate-900 shadow-xl">
            <form id="mainForm" class="grid grid-cols-1 md:grid-cols-7 gap-4 items-end">
                <div class="md:col-span-1">
                    <label class="block text-xs font-black text-slate-700 mb-2">交易日期</label>
                    <input type="date" id="entryDate" class="w-full">
                </div>
                <div class="md:col-span-1">
                    <label class="block text-xs font-black text-slate-700 mb-2">交易類型</label>
                ...
                <div class="md:col-span-1">
                    <label class="block text-xs font-black text-slate-700 mb-2">股票代號</label>
                ...
                <div id="tradeFields" class="md:col-span-2 grid grid-cols-2 gap-2">
                    <div>
                        <label class="block text-xs font-black text-slate-700 mb-2">成交單價</label>
                        <input type="number" id="price" step="0.001" class="w-full">
                    </div>
                    <div>
                        <label class="block text-xs font-black text-slate-700 mb-2">成交數量</label>
                        <input type="number" id="quantity" class="w-full">
                    </div>
                </div>
                <div class="md:col-span-1">
                    <label class="block text-xs font-black text-slate-700 mb-2">附加金額</label>
                    <input type="number" id="feeOrAmount" step="0.01" value="0" class="w-full font-black text-blue-700 bg-blue-50">
                </div>
                <div class="md:col-span-1">
                    <button type="submit" class="w-full bg-blue-600 text-white py-3 rounded-md hover:bg-blue-700 font-black transition shadow-md">新增紀錄</button>
                </div>
                <div class="md:col-span-7">
                    <input type="text" id="note" placeholder="輸入備註事項..." class="w-full italic text-sm">
                </div>
            </form>
        </div>

        <div class="flex flex-col md:flex-row justify-between items-center mb-6 px-2 gap-4">
            <h2 class="font-black text-slate-800 text-xl flex items-center gap-2">
                📜 明細清單 <span id="filterLabel" class="hidden bg-slate-200 text-slate-700 px-3 py-1 rounded-full text-sm font-mono"></span>
            </h2>
            <div class="w-full md:w-96">
                <input type="text" id="filterInput" onkeyup="render()" placeholder="🔍 輸入代號查看統計 (例如: 2330)" class="w-full !border-2 !border-amber-400 !bg-white p-3 rounded-xl shadow-sm font-bold text-slate-800">
            </div>
        </div>

        <div class="card overflow-hidden shadow-2xl border border-slate-200">
                ...
                <table class="min-w-full text-sm">
                    <thead>
                        <tr class="force-header">
                            ...
                            <th class="px-6 py-4 text-right">交易詳情</th>
                            <th class="px-6 py-4 text-right">附加金額</th>
                            <th class="px-6 py-4 text-right">現金流</th>
    ...
    <script>
        ...
        function render() {
            ...
            if (filter) {
                ...
                statsGrid.innerHTML = `
                    <div class="card p-5 border-b-4 border-blue-600">
                        <p class="text-[10px] font-black text-slate-400 uppercase">目前持倉股數</p>
                        <p class="text-2xl font-black ${currentQty > 0 ? 'text-blue-700' : 'text-slate-400'}">${currentQty.toLocaleString()}</p>
                    </div>
                    <div class="card p-5 border-b-4 border-amber-500">
                        <p class="text-[10px] font-black text-slate-400 uppercase">平均買入成本</p>
                    ...
                    <div class="card p-5 border-b-4 border-slate-300">
                        <p class="text-[10px] font-black text-slate-400 uppercase">累積附加 (息/費)</p>
                    ...
                    <div class="card p-5 bg-slate-900 text-white shadow-xl">
                        <p class="text-[10px] font-bold text-slate-400 uppercase">該標的總淨流</p>
                ...
                statsGrid.innerHTML = `
                    <div class="card p-5 border-b-4 border-slate-400">
                        <p class="text-[10px] font-black text-slate-400 uppercase">全帳戶流水淨值</p>
                        <p class="text-2xl font-black text-slate-800">$${totalCashFlow.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
                    </div>
                    <div class="card p-5 border-b-4 border-red-600">
                        <p class="text-[10px] font-black text-slate-400 uppercase">總計支出費用</p>
                        <p class="text-2xl font-black text-red-600">$${totalFees.toLocaleString()}</p>
                    </div>
                    <div class="md:col-span-2 card p-5 bg-slate-900 text-white flex flex-col justify-center border-b-4 border-amber-500">
                        <p class="text-xs font-black text-amber-400 uppercase tracking-widest">請輸入代號查看持倉統計</p>
                    </div>
                `;
            }
        }
        ...
        function renderNormalRow(r, cash) {
            ...
            return `
                <tr class="hover:bg-slate-50 transition-colors ${borderClass}">
                    <td class="px-6 py-4 font-mono font-bold text-slate-600">${r.date}</td>
                    <td class="px-6 py-4 font-black ${colorClass}">${r.type}</td>
                    <td class="px-6 py-4 font-black text-slate-900">${r.symbol}</td>
                    <td class="px-6 py-4 text-right text-slate-700 font-bold">${r.price ? `${r.price.toLocaleString()} × ${r.quantity.toLocaleString()}` : '--'}</td>
                    <td class="px-6 py-4 text-right font-black ${r.feeOrAmount < 0 ? 'text-red-600' : 'text-green-600'}">
                    ...
                    <td class="px-6 py-4 text-center">
                        <button onclick="startEdit(${r.id})" class="text-slate-400 hover:text-blue-600 mr-4 font-bold text-lg">✎</button>
                        <button onclick="deleteRecord(${r.id})" class="text-slate-300 hover:text-red-600 font-bold text-lg">✕</button>
                    </td>
                </tr>
                ${r.note ? `<tr><td colspan="7" class="px-12 py-2 text-[12px] text-slate-600 italic bg-slate-50/50 border-none font-bold">備註：${r.note}</td></tr>` : ''}
            `;
        }
        ...
        function exportCSV() { let csv = "\uFEFF日期,類型,代號,單價,數量,附加金額,備註\n"; records.forEach(r => csv += `${r.date},${r.type},${r.symbol},${r.price},${r.quantity},${r.feeOrAmount},${r.note}\n`); const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' }); const link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.download = `Portfolio_Tracker.csv`; link.click(); }
